...
def email_csc():
    ...
    arquivo_anexo = r"G:\Drives compartilhados\Fiscal_Arquivo_de_Notas\FISCAL_MATERIAL\CONTROLE GRC - MATERIAIS\Célula Fiscal GRC - Materiais.XLSX"

    df_emails_filtrados = [
        "cassiano.deluca@rogga.com.br",
        "daiane.noskoski@rogga.com.br",
        "maria.vanderlinder@rogga.com.br",
        "alisson.streis@rogga.com.br",
   ...
   # 📌 **Verificação se o arquivo realmente existe antes de anexá-lo**
    if not os.path.isfile(arquivo_anexo):
        ...
        try:
            # Detecta o tipo MIME do arquivo automaticamente
            tipo_mime, _ = mimetypes.guess_type(arquivo_anexo)
            if tipo_mime is None:
                tipo_mime = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"  # Tipo correto para XLSX

            # Lendo o arquivo em modo binário
            with open(arquivo_anexo, "rb") as anexo:
                parte_anexo = MIMEBase(*tipo_mime.split("/"))
            ...
            print(f"📎 Arquivo anexado corretamente: {filename}")
    ...
    try:
        with smtplib.SMTP("smtp.gmail.com", 587) as servidor:
            ...
            servidor.sendmail(email_remessa, emails_list, msg.as_string())
...
def email_obras():
    ...
    for index, row in df_emails_obras.iterrows():
        ...
        nome_arquivo_base = str(int(float(row["Local"]))).strip()
